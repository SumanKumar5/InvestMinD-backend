    ...
    if (holding) {
      if (type === "buy") {
        ...
        const newAvgPrice =
          (holding.avgBuyPrice * holding.quantity +
            numericAvgBuyPrice * numericQuantity) /
          totalQty;
        ...
        if (req.body.companyName) holding.companyName = req.body.companyName.trim();
        await holding.save();
      ...
      if (type === "sell") {
        if (numericQuantity > holding.quantity) {
          return res
            .status(400)
            .json({ message: "Sell quantity exceeds available holdings" });
        }
        ...
        if (holding.quantity === 0) {
          ...
          return res
            .status(200)
            .json({ message: `All ${symbol} sold. Holding deleted.` });
        }
      ...
      if (type === "sell") {
        return res
          .status(400)
          .json({ message: `You don't own any ${symbol} to sell.` });
      }

      

      // New holding creation: use companyName from frontend or fallback
      let companyName = req.body.companyName?.trim() || symbol;
...
exports.deleteHolding = async (req, res) => {
  try {
    ...
    if (
      !holding ||
      holding.portfolio.user.toString() !== req.user._id.toString()
    ) {
      return res
        .status(403)
        .json({ message: "Unauthorized or holding not found" });
    }
    ...
    const enrichedHoldings = await Promise.all(
      holdings.map(async (h) => {
        ...
        const gainLoss = Number(
          ((currentPrice - numericAvgBuyPrice) * numericQuantity).toFixed(2)
        );
        const gainLossPercent = Number(
          (
            ((currentPrice - numericAvgBuyPrice) / numericAvgBuyPrice) *
            100
          ).toFixed(2)
        );
